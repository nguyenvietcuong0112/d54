...
//      callToActionView.setBackground(ctaBackground);
      if (ctaBackground instanceof ColorDrawable) {
        int color = ((ColorDrawable) ctaBackground).getColor();
        ...
        roundedDrawable.setCornerRadius(radiusPx);
        callToActionView.setBackground(roundedDrawable);
      } else if (ctaBackground != null) {
        // Nếu là loại drawable đã có sẵn góc, set luôn
        callToActionView.setBackground(ctaBackground);
      } else {
        // fallback nếu null
        callToActionView.setBackground(null);
      }
    }
